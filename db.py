...
def fetch_inventory():
  ...
  COALESCE(MAX(r.days_unsold), DATEDIFF(CURDATE(), COALESCE(MAX(s.sale_date), p.added_date))) AS `Days Unsold`,
  MAX(r.risk_level) AS `Risk Level`,
  MAX(r.risk_score) AS `Risk Score (%)`,
  MAX(r.suggested_action) AS `Suggested Action`
FROM products p
